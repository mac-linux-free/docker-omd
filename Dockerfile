...
RUN apt-get update && \
    ...
    wget --no-check-certificate https://mathias-kettner.de/support/1.4.0p9/check-mk-raw-1.4.0p9_0.stretch_amd64.deb && \
    (dpkg -i check-mk-raw-1.4.0p9_0.stretch_amd64.deb || true) && \
    apt-get install -y -f --no-install-recommends  && \
    apt-get clean && rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/* && \
    rm -rf check-mk-raw-1.4.0p9_0.stretch_amd64.deb
